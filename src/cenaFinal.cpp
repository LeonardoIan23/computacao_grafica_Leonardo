...
public:
    ...
    Camera(vec3 pos = vec3(0.f, 2.f, 8.f),
           ...
           float y = -90.f,
           float p = 0.f)
        : position(pos), worldUp(wUp), yaw(y), pitch(p), speed(5.f), sensitivity(0.1f)
    {
        updateVectors();
    }
...
{
    float u = 1.f - t;
    return u * u * u * p[0] + 3.f * u * u * t * p[1] + 3.f * u * t * t * p[2] + t * t * t * p[3];
}
                            ...
                            const vec3 &nrm, int &outNVerts)
{
    ...
    push(0, 0);
    push(1, 1);
    push(2, 2); // triangulo 1: BL, BR, TR
    push(3, 0);
    push(4, 2);
    push(5, 3); // triangulo 2: BL, TR, TL
...
static void setupEnvPlanes(const json &j)
{
    ...
    // ── PAREDE LATERAL ESQUERDA (X=-W) – dividida em 3 partes + fundo de tunel ──
    {
        ...
        auto pushL = [&](float z0, float z1, float y0, float y1)
        {
            ...
            EnvPlane ep;
            ep.VAO = createQuadVAO(v, u, nrm, ep.nVerts);
            ep.textureID = wallTexID;
            envPlanes.push_back(ep);
        };
        ...
        EnvPlane dark;
        vec3 dv[4] = {{-W - 0.3f, 0.f, TUN_Z_MIN}, {-W - 0.3f, 0.f, TUN_Z_MAX}, {-W - 0.3f, TUN_Y_MAX, TUN_Z_MAX}, {-W - 0.3f, TUN_Y_MAX, TUN_Z_MIN}};
        vec2 du[4] = {{0, 0}, {1, 0}, {1, 1}, {0, 1}};
        dark.VAO = createQuadVAO(dv, du, {1.f, 0.f, 0.f}, dark.nVerts);
        dark.textureID = whiteTex();
        dark.Ka = {0.04f, 0.04f, 0.05f};
        dark.Kd = {0.f, 0.f, 0.f};
        dark.Ks = {0.f, 0.f, 0.f};
        dark.Ns = 1.f;
...
bool setupScene(const json &j)
{
    ...
    if (j.contains("camera"))
    {
        ...
        if (c.contains("yaw"))
            camera.yaw = c["yaw"].get<float>();
        if (c.contains("pitch"))
            camera.pitch = c["pitch"].get<float>();
    ...
    for (const auto &jo : j["objects"])
    {
        ...
        if (objPath.empty())
        {
            cerr << "  Objeto '" << name << "' sem campo 'obj'\n";
            continue;
        }
...
int main()
{
    ...
    if (sceneJson.contains("window"))
    {
        ...
        if (w.contains("width"))
            gWinW = w["width"].get<int>();
        if (w.contains("height"))
            gWinH = w["height"].get<int>();
        if (w.contains("title"))
            winTitle = w["title"].get<string>();
    }
    lastX = gWinW / 2.f;
    lastY = gWinH / 2.f;
    ...
    if (sceneJson.contains("camera"))
    {
        ...
        if (c.contains("fov"))
            fov = c["fov"].get<float>();
        if (c.contains("near"))
            nearP = c["near"].get<float>();
        if (c.contains("far"))
            farP = c["far"].get<float>();
    }
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        {
            if (objLeft)
                sel.position.x -= OBJ_SPEED * dt;
            if (objRight)
                sel.position.x += OBJ_SPEED * dt;
            if (objFwd)
                sel.position.z -= OBJ_SPEED * dt;
            if (objBack)
                sel.position.z += OBJ_SPEED * dt;
        }
        // Y sempre disponivel
        if (objPgUp)
            sel.position.y += OBJ_SPEED * dt;
        if (objPgDn)
            sel.position.y -= OBJ_SPEED * dt;
        if (keyScaleUp)
            sel.scaleXYZ *= (1.f + SCALE_SPEED * dt);
        if (keyScaleDown)
            sel.scaleXYZ *= std::max(1.f - SCALE_SPEED * dt, 0.01f);
...
void key_callback(GLFWwindow *window, int key, int scancode, int action, int mode)
{
    ...
    if (key == GLFW_KEY_W)
        camW = (action != GLFW_RELEASE);
    if (key == GLFW_KEY_S)
        camS = (action != GLFW_RELEASE);
    if (key == GLFW_KEY_A)
        camA = (action != GLFW_RELEASE);
    if (key == GLFW_KEY_D)
        camD = (action != GLFW_RELEASE);
    if (key == GLFW_KEY_SPACE)
        camUp = (action != GLFW_RELEASE);
    if (key == GLFW_KEY_C)
        camDown = (action != GLFW_RELEASE);
    ...
    if (key == GLFW_KEY_LEFT)
        objLeft = (action != GLFW_RELEASE);
    if (key == GLFW_KEY_RIGHT)
        objRight = (action != GLFW_RELEASE);
    if (key == GLFW_KEY_UP)
        objFwd = (action != GLFW_RELEASE);
    if (key == GLFW_KEY_DOWN)
        objBack = (action != GLFW_RELEASE);
    if (key == GLFW_KEY_PAGE_UP)
        objPgUp = (action != GLFW_RELEASE);
    if (key == GLFW_KEY_PAGE_DOWN)
        objPgDn = (action != GLFW_RELEASE);
...
int loadSimpleOBJ(const string &filePath, int &nVertices, Material &outMat)
{
    ...
    if (sl != string::npos)
        baseDir = filePath.substr(0, sl + 1);
    ...
    while (getline(file, line))
    {
        ...
        else if (t == "f")
        {
            ...
            while (ss >> word)
            {
                ...
                string idx;
                if (getline(ws, idx, '/'))
                    vi = idx.empty() ? 0 : stoi(idx) - 1;
                if (getline(ws, idx, '/'))
                    ti = idx.empty() ? 0 : stoi(idx) - 1;
                if (getline(ws, idx))
                    ni = idx.empty() ? 0 : stoi(idx) - 1;
            ...
            {
                vBuffer.push_back(fv.pos.x);
                vBuffer.push_back(fv.pos.y);
                vBuffer.push_back(fv.pos.z);
                vBuffer.push_back(fv.uv.s);
                vBuffer.push_back(fv.uv.t);
                vBuffer.push_back(fv.nrm.x);
                vBuffer.push_back(fv.nrm.y);
                vBuffer.push_back(fv.nrm.z);
            };
