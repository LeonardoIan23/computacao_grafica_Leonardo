...
/* Visualizador 3D - Cubos com transformações
 *
 * Baseado no projeto de Rossana Baptista Queiroz
 * Implementação: cubo colorido, translação, escala e múltiplas instâncias
...
// Vertex Shader — inclui view e projection para perspectiva real
...
int main()
{
    ...
    // --- Projeção perspectiva (45 graus de FOV) ---
    mat4 projection = perspective(radians(45.0f), (float)WIDTH / (float)HEIGHT, 0.1f, 100.0f);
    glUniformMatrix4fv(projectionLoc, 1, GL_FALSE, value_ptr(projection));

    // --- Câmera posicionada acima e atrás, olhando para a origem ---
    mat4 view = lookAt(
        vec3(0.0f, 1.5f, 4.5f), // posição da câmera
    ...
    // --- 3 cubos com rotação inicial para já aparecerem em ângulo 3D ---
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        for (int i = 0; i < (int)cubes.size(); i++)
        {
            ...
            // Acumula ângulo de rotação contínua
            if (cube.rotateX)
...
int setupGeometry()
{
    ...
    return VAO;
>>>>>>> c84b45f541b0c839ac5d525a46ff73ed81aec41d
}