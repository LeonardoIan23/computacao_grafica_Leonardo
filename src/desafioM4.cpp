/* Desafio Modulo 4 - Modelo de Iluminacao de Phong
 * Extende o desafioM3 adicionando:
 *   - Leitura dos coeficientes Ka, Kd, Ks e Ns do arquivo .MTL
 *   - Envio dos coeficientes como uniforms por objeto ao fragment shader
 *   - Calculo completo de Phong (ambiente + difusa + especular) por pixel
...
struct Material {
    vec3  Ka = vec3(0.2f);   // coeficiente ambiente (default)
    vec3  Kd = vec3(0.8f);   // coeficiente difuso   (default)
    vec3  Ks = vec3(0.5f);   // coeficiente especular (default)
    float Ns = 32.0f;        // expoente de brilho    (default)
    string texturePath;
};
...
void main() {
    ...
    // Matriz normal correta para translacoes e escalas nao uniformes
...
// ============================================================
//  Fragment Shader - Phong completo com coeficientes do MTL
//
...
// Posicoes da luz e camera no espaco mundo
...
// Fator de destaque visual do objeto selecionado
...
// ============================================================
//  Helper: carrega um OBJ e adiciona ao vector de objetos
// ============================================================
...
int main()
{
    ...
    GLFWwindow* window = glfwCreateWindow(WIDTH, HEIGHT,
        "Desafio M4 - Phong Shading", nullptr, nullptr);
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        for (int i = 0; i < (int)objects.size(); i++)
        {
            ...
            // Coeficientes de material do objeto (lidos do MTL)
...
// ============================================================
//  parseMTL
//  Le um arquivo .MTL e extrai:
//    - Ka  (coeficiente ambiente  - vec3)
//    - Kd  (coeficiente difuso    - vec3)
//    - Ks  (coeficiente especular - vec3)
//    - Ns  (expoente de brilho    - float)
//    - map_Kd (caminho da textura difusa)
// ============================================================
Material parseMTL(const string& mtlPath)
{
    Material mat; // ja vem com valores default
    ...
    // Diretorio base para resolver caminhos relativos de textura
    string baseDir;
    ...
    while (getline(file, line))
    {
        ...
        if (token == "Ka") {
            ...
            string texName;
            ss >> texName;
            mat.texturePath = baseDir + texName;
        }
    }
...
//
//  Tambem le o arquivo .MTL referenciado e preenche outMat com
//  os coeficientes de iluminacao e o caminho da textura.
//
...
int loadSimpleOBJ(const string& filePath, int& nVertices, Material& outMat)
{
    ...
    while (getline(file, line))
    {
        ...
        if (token == "v") {
            ...
            outMat = parseMTL(baseDir + mtlFile);
        }
        else if (token == "f") {
            string word;
            while (ss >> word) {
